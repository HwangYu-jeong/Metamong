...
public class World_Navigation : MonoBehaviourPunCallbacks
{
    ...
    {
        GameObject[] a_house = Resources.LoadAll<GameObject>("photon_house_prefab/A");
        GameObject[] b_house = Resources.LoadAll<GameObject>("photon_house_prefab/B");
        GameObject[] c_house = Resources.LoadAll<GameObject>("photon_house_prefab/C");
    ...
    IEnumerator Create_House(float delay)
    {
        ...
        for (int i = 0; i < ac.h_position_index.Count; i++) {
            if (ac.h_is_house[i] == true) {
                int j = ac.h_position_index[i] - 1;
                nullList[j] = -1;
            }
        }

        // 메인 월드에 구역에 맞는 House 외형 및 건설된 상담소 건물 생성
        for (int i = 0; i < nullList.Count; i++) {
            if (0 <= nullList[i] && nullList[i] <= 14)
            {
                Vector3 portal = All_Position[nullList[i]].localPosition + 
                    (All_Position[nullList[i]].forward * 15.0f) - All_Position[nullList[i]].up * 10.0f;
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/A/A_0",
                    All_Position[nullList[i]].localPosition - new Vector3(0f, 10f, 0f), forcus);
            }
            ...
            else if (15 <= nullList[i] && nullList[i] <= 26)
            {
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/B/B_0", All_Position[nullList[i]].localPosition - new Vector3(0f, 10f, 0f), forcus);
            } 
            ...
            else if (27 <= nullList[i] && nullList[i] <= 42)
            {
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/C/C_0", All_Position[nullList[i]].localPosition - new Vector3(0f, 10f, 0f), forcus);
            }
        }

        // 개설된 상담소의 House 외형 적용시켜 상담소 건물 생성 
        for (int i = 0; i < ac.h_position_index.Count; i++) {
            if (0 <= ac.h_position_index[i] && ac.h_position_index[i] <= 14 && ac.h_is_house[i]) 
            {
                Vector3 portal = All_Position[ac.h_position_index[i] - 1].localPosition +
                    (All_Position[ac.h_position_index[i] - 1].forward * 15.0f) - All_Position[ac.h_position_index[i] - 1].up * 10.0f;
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/A/" +
                    A_House_Prefab[ac.h_house_index[i]].name, All_Position[ac.h_position_index[i] - 1].localPosition - new Vector3(0f, 10f, 0f), forcus);
            } 
            ...
            else if (15 <= ac.h_position_index[i] && ac.h_position_index[i] <= 26 && ac.h_is_house[i]) 
            {
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/B/" + B_House_Prefab[ac.h_house_index[i]].name, All_Position[ac.h_position_index[i] - 1].localPosition - new Vector3(0f, 10f, 0f), forcus);
            } 
            ...
            else if (27 <= ac.h_position_index[i] && ac.h_position_index[i] <= 42 && ac.h_is_house[i])
            {
                ...
                GameObject house = PhotonNetwork.Instantiate("photon_house_prefab/C/" + C_House_Prefab[ac.h_house_index[i]].name, All_Position[ac.h_position_index[i] - 1].localPosition - new Vector3(0f, 10f, 0f), forcus);
            }
        }
    }
